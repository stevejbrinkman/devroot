...
struct Grid
{
    ...
    var frameReductionRate: CGFloat = 0.0
    
    init(layout: Layout, frame: CGRect = CGRect.zero, reductionRate: CGFloat = 0.0) {
        self.frame = frame
        self.layout = layout
        self.frameReductionRate = reductionRate
        recalculate()
    }
    ...
    subscript(index: Int) -> CGRect? {
        return index < cellFrames.count ? reduceAndCenterCellFrame(forRect: cellFrames[index]) : nil
    }
    ...
    private func reduceAndCenterCellFrame(forRect: CGRect) -> CGRect{
        // This function puts a margin between the actual size of the grid frame and the view that will be rendered
        let factor = (1.0 - frameReductionRate)
        let orig = CGPoint(x: forRect.minX, y: forRect.minY)
        let rectSize = CGSize(width: forRect.width * factor, height: forRect.height * factor)
        let returnRect = CGRect(origin: orig, size: rectSize)
        return returnRect
    }
    ...
    private func cellSizeAssuming(rowCount: Int? = nil, columnCount: Int? = nil, minimumAllowedSize: CGSize = CGSize.zero) -> CGSize {
        ...
        /*var returnSize: CGSize = CGSize.zero
        returnSize.width = minimumAllowedSize.width * (1.0 + frameReductionRate)
        returnSize.height = minimumAllowedSize.height * (1.0 + frameReductionRate)
        return returnSize*/
